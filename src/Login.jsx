    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-[#0a1628] via-[#0f2140] to-[#0a1628] flex items-center justify-center p-4 sm:p-6 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10 py-6">
        ...
        <div className="flex flex-col items-center mb-5 text-center">
          ...
          <h1 className="text-white text-2xl font-semibold tracking-tight">My Budget Tracker</h1>
          <p className="text-[#d4af5f]/80 text-sm font-medium tracking-[0.2em] mt-1.5">PLAN &middot; TRACK &middot; SAVE &middot; GROW</p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-white/[0.05] border border-[#d4af5f]/20 rounded-2xl px-4 py-3 mb-5">
          <div className="w-9 h-9 rounded-full bg-[#d4af5f]/15 flex items-center justify-center shrink-0">
            <Store size={18} className="text-[#d4af5f]" />
          </div>
          <div className="min-w-0">
            <p className="text-white text-base font-medium truncate">Ad Bhutta &amp; Brothers</p>
            <div className="flex items-center gap-1 text-[#8fa5c4] text-sm">
              <Phone size={13} /> By Ad Bhutta &middot; 0321-6101060
            </div>
          </div>
        </div>
        ...
        {savedBio && (
          <div className="mb-5">
            <button
              ...
              disabled={bioBusy}
              className="w-full flex items-center justify-center gap-2 bg-[#d4af5f]/10 border border-[#d4af5f]/40 text-[#d4af5f] py-3 rounded-2xl text-base font-semibold disabled:opacity-60"
            >
              <Fingerprint size={20} />
              {bioBusy ? "Verifying..." : `Unlock with Face ID / Fingerprint`}
            </button>
            <div className="flex items-center justify-between mt-2 px-1">
              <p className="text-sm text-[#5c7398] truncate">{savedBio.email}</p>
              <button onClick={forgetBiometric} className="text-sm text-[#5c7398] hover:text-rose-300 flex items-center gap-0.5 shrink-0">
                <X size={13} /> Remove
              </button>
            </div>
            {bioError && <p className="text-sm text-rose-300 mt-2 bg-rose-500/10 border border-rose-500/25 rounded-lg px-3 py-2">{bioError}</p>}
            <div className="flex items-center gap-3 my-4">
              <div className="flex-1 h-px bg-white/10"></div>
              <span className="text-sm text-[#5c7398]">or use email &amp; password</span>
              <div className="flex-1 h-px bg-white/10"></div>
            </div>
          </div>
        )}
        ...
        <div className="w-full bg-white/[0.05] backdrop-blur-xl border border-[#d4af5f]/15 rounded-3xl p-6 sm:p-7 shadow-2xl shadow-black/40">
          <div className="flex items-center gap-2 text-white mb-1">
            <Wallet size={20} className="text-[#d4af5f]" />
            <p className="text-lg font-medium">{mode === "login" ? "Welcome back" : "Create your account"}</p>
          </div>
          <p className="text-sm text-[#8fa5c4] mb-6">
            {mode === "login" ? "Sign in to continue your financial journey" : "Set up your new personal account"}
          </p>

          <div className="space-y-3 mb-4">
            <div className="relative">
              <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#5c7398]" />
              <input
                type="email" placeholder="Email address" value={email}
                onChange={e => setEmail(e.target.value)}
                className="w-full bg-white/[0.06] border border-[#d4af5f]/15 rounded-xl pl-10 pr-3 py-3 text-base text-white placeholder-[#5c7398] outline-none focus:border-[#d4af5f]/60 focus:bg-white/[0.09] transition"
              />
            </div>
            <div className="relative">
              <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#5c7398]" />
              <input
                type={showPw ? "text" : "password"} placeholder="Password" value={password}
                onChange={e => setPassword(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter") submit(); }}
                className="w-full bg-white/[0.06] border border-[#d4af5f]/15 rounded-xl pl-10 pr-10 py-3 text-base text-white placeholder-[#5c7398] outline-none focus:border-[#d4af5f]/60 focus:bg-white/[0.09] transition"
              />
          ...
          {bioSupported && (
            <label className="flex items-center gap-2 mb-4 cursor-pointer">
              ...
              <span className="text-sm text-[#8fa5c4] flex items-center gap-1"><Fingerprint size={15}/> Enable Face ID / Fingerprint login on this device</span>
            </label>
          )}

          {error && <p className="text-sm text-rose-300 mb-3 bg-rose-500/10 border border-rose-500/25 rounded-lg px-3 py-2">{error}</p>}

          <button
            onClick={submit}
            disabled={busy}
            className="w-full bg-gradient-to-r from-[#d4af5f] to-[#c49a45] hover:from-[#e0bd6f] hover:to-[#d4af5f] text-[#0a1628] py-3 rounded-xl text-base font-semibold disabled:opacity-60 transition shadow-lg shadow-[#d4af5f]/20"
          >
            {busy ? "Please wait..." : mode === "login" ? "Log in" : "Sign up"}
          </button>

          <p className="text-sm text-[#8fa5c4] text-center mt-5">
